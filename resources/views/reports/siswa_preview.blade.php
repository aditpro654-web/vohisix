...
@section('title', 'Preview Laporan Siswa PKL')
...
<div class="card" style="padding: 24px; max-width: 1200px; margin: 0 auto;">
    <div style="display: flex; flex-wrap: wrap; justify-content: space-between; align-items: center; gap: 16px; margin-bottom: 24px;">
        <div>
            <h1 style="margin:0; font-size:1.75rem; color:#0f172a;">Preview Laporan Siswa PKL</h1>
            <p style="margin:8px 0 0; color:#475569; font-size:0.95rem; max-width:720px;">Periksa ringkasan dan daftar siswa sebelum mengunduh PDF berformat profesional.</p>
        </div>
        <div style="display:flex; flex-wrap:wrap; gap:10px;">
            <a href="{{ $downloadUrl }}" target="_blank" rel="noopener" class="btn btn-primary">Unduh PDF</a>
            ...
            <tbody>
                @forelse($siswas as $index => $siswa)
                    ...
                    <tr style="background: {{ $index % 2 === 0 ? '#ffffff' : '#f8fafc' }};">
                        ...
                        <td style="padding:12px 12px; border-bottom:1px solid #e2e8f0;"><span style="display:inline-flex; align-items:center; gap:6px; padding:6px 10px; border-radius:9999px; font-size:0.75rem; font-weight:700; color:{{ $status === 'Diterima' ? '#047857' : ($status === 'Ditolak' ? '#991b1b' : '#92400e') }}; background:{{ $status === 'Diterima' ? '#d1fae5' : ($status === 'Ditolak' ? '#fee2e2' : '#fef3c7') }};">{{ $status }}</span></td>
    ...
    <div style="margin-top:24px; display:flex; flex-wrap:wrap; gap:12px; justify-content:space-between; align-items:center; color:#475569;">
        <div style="max-width:760px; font-size:0.95rem; line-height:1.75;">
            Halaman preview ini menampilkan data sesuai filter yang dipilih. Klik "Unduh PDF" untuk mendapatkan laporan dengan layout profesional, atau gunakan "Preview + Unduh Otomatis" untuk langsung memicu unduhan sekaligus melihat preview.
        </div>
